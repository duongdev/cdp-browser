...
- **D — Search BFF contract + route (server).** `POST /api/chat/search` (service-agnostic, `service` discriminator): parse KQL filters (pure `search-query.ts` parser, TDD), call substrate, hydrate-on-render (background, capped concurrency), return ranked hit rows (Relevance|Recent) + scope handling via `listScopes`.

  **API contract (decided 2026-07-28):** one request-response — **no SSE**. Request: `{ service, query, sort?: "relevance"|"recent", scope?, cursor? }`. Response: `{ rows: SearchHit[], parsed: ParsedQuery, cursor, degraded? }` where `SearchHit = { convId, msgId, ts, sender, convTitle, snippet, source: "local"|"substrate", hydrated: boolean }` — rows merged local FTS + substrate, deduped by `(convId,msgId)`; `parsed` is the echo the FE renders filter chips from; `degraded` marks a local-only result when substrate 401/429s. Async part is hydrate completion only, and it rides the **existing WS delta hub**: hydrate goes through `store.upsertMessages` → the existing `messages-upsert` delta fires → the search view (already on `/api/chat/ws`) flips `hydrated:false` rows in place. No new transport; a `hydrate-progress` WS frame (mirroring `backfill-progress`) is an optional add only if row-flip UX proves insufficient. If first-paint latency ever matters, the FE issues two parallel POSTs (local-only, then substrate) — still plain HTTP.
...
- [ ] `POST /api/chat/search` parses KQL filters (`from/in/after/before/has/mentions:me`), honors Relevance⇄Recent + scope chips, returns ranked rows `{convId,msgId,ts,sender,convTitle,snippet,source,hydrated}` + `parsed` echo + `degraded` on upstream failure. One request-response, no SSE. Parser unit-tested.
- [ ] Background hydrate completion reaches the open search view via the existing `messages-upsert` WS delta and flips `hydrated:false` rows in place — no new transport.
