...
- **BFF DB** (service-agnostic): `conversations(service, id, kind, title, last_message_*, sync cursors)`, `messages(service, conv_id, id, sender, ts, body_html, reactions, attachments, deleted, edited, mentions_me)`, `read_state`, `prefs(labels/folder/mute/mutedUntil/notifyOnMention/customTitle)`, `settings`, `users`, `push_subs`. Keys prefixed by `service` so a second service is additive.
