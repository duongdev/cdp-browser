# 114 — teams chat conversation list: infinite scroll (auto load-more on scroll)

- **Status:** done
- **Mode:** HITL
- **Depends on:** t112 (list cursor paging)
...
- Mechanism: a bottom **`IntersectionObserver` sentinel** (root = viewport, `rootMargin: "400px"`
  ...
  per appended row. The dedup-on-append + fail→null-cursor behavior from t112 is unchanged; the
...
- **Layer 1:** n/a — no new pure logic (effectful observer wiring; the paging reducer is t112's).
...
- Thread-history infinite scroll (already scroll-to-top auto-loads since t112).
