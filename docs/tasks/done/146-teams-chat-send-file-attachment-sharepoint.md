# 146 — teams chat: send a file attachment (non-image) via SharePoint/OneDrive upload

- **Status:** done
- **Mode:** HITL
- **Depends on:** t130 (send), t141 (file-attachment READ/chips), t145 (send-image / upload plumbing)

## Goal

Attach and send an arbitrary file (PDF, doc, screenshot-as-file, etc.) in the composer. Unlike an
inline image (AMS, t145), a **file** uploads to the user's **OneDrive/SharePoint** "Microsoft Teams
Chat Files" folder and is posted as a file chiclet — the same `properties.files` shape t141 already
...
createLink 201, send 201, t141 render + open confirmed, cleanup delMsg 200 / delFile 204. No
user-assisted capture was needed; the driveItem + a createLink are enough. All steps run IN-PAGE
(CA-proof), like t145's image upload. No `addStub` / `StartUpload` / `driveItem/commands` — those in the
...
`fileChicletState:{serviceName:"p2p",state:"active"}`) are exactly what t141's `parseFiles` reads back.
...
- **`web/server.mjs` `POST /api/teams/upload-file`** `{ convId, filename, base64, contentType, text? }` →
  ...
  Size limit like t145's `IMAGE_BODY_LIMIT` (files can be large — consider a higher cap + a client guard;
  for v1 cap at a sane size, e.g. 24–50MB, and reject bigger with an honest error).

### Client
- **`chat/src/components/thread-view.tsx`** composer: extend the t145 pending-attachment slot to accept
  ...
  → poll reconciles to the server `properties.files` chip (t141). Reuse the pending-attachment plumbing
  from t145 (generalize "pending image" → "pending attachment" = image | file).
- **`chat/src/lib/teams-client.ts`**: `uploadFile(convId, file, text?)`.

## Acceptance criteria

- [x] Attaching a file (e.g. a PDF) and sending it posts a file chiclet that renders (t141 chip) and
...
- [x] Image paste (t145) still works; the pending slot cleanly handles image vs file (`pendingFile`).

## Test plan

- **Layer 1 (TDD)**: pure helpers — the `properties.files` builder (from driveItem + shareUrl + filename),
  the `{userPath}`/URL builders, the file-type→icon mapping (may already exist from t141).
...
- Reuses t141's file-chip RENDER (this is the SEND half). No new ADR (within ADR-0019).
- The AMS image path (t145) and this SharePoint file path are DISTINCT — Teams itself ran both when the
...
- **`chat/src/components/thread-view.tsx`** — `pendingImage` → `pendingFile` (image thumbnail | file chip);
  ...
  (its t141 `AttachmentChip` already renders the optimistic file chip).
