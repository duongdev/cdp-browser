...
- **Status:** in-progress (2026-06-19: prod LIVE on portal.dp.dustin.one, **cutover DONE**, **PR previews verified end-to-end**; only iPad PWA re-install + ADR-0014 remain)
- **Mode:** HITL
- **Estimate:** 1d
...
- **Execution log (2026-06-19, AFK via REST/SSH — no Dokploy MCP):**
  ...
  - ✅ **PR previews VERIFIED end-to-end (2026-06-19)**: PR #7 (throwaway, closed) auto-created a preview at `preview-cdp-browser-app-1yrpdy-3hcus0.dp.dustin.one` — built the branch (served sha `e353528` vs prod `7910b20`), reachable over tailnet with trusted TLS, and PR close tore it down (deployment gone, URL 404, swarm service removed). So the Dokploy GitHub App **is** subscribed to `pull_request` events.
  - ✅ **previewEnv fix (non-obvious Dokploy gotcha)**: previews do NOT inherit the Application's `env` — `previewEnv` was empty, so the first preview's `/api/config` returned `localhost` (no CDP host). Set `previewEnv = CDP_HOST=<glkvm>\nCDP_PORT=9222\nPORT=7800\nAPP_TITLE=CDP preview`; redeployed → `/api/config` then returned the glkvm host. Future previews inherit it.
  - ✅ **Cutover DONE (2026-06-19)**: old Compose `cdp-browser` (id `Fru7-...`) stopped + deleted (`deleteVolumes:false` — old `cdp-web-data` volume kept on dell01, prune later if wanted); `tailscale serve :8443` removed (`No serve config`). Prod is now solely `portal.dp.dustin.one` on the Application; old `ts.net:8443` endpoint is dead; single `web/server.mjs` sweeper (no more double-deploy / double notification capture).
  - **HITL remaining**: (1) iPad — re-install the PWA on `https://portal.dp.dustin.one` + re-grant push (the old `:8443` origin is gone). (2) ADR-0014 (task DoD) + append it to `docs/conventions/docs-discipline.md`'s ADR list; ADR-0007 still references the old `portal.dustin.one` (append-only — supersede, don't edit).
