...
describe("shouldNotifyOs", () => {
  ...
  it("fires when enabled and the capturing tab is in the background", () => {
    expect(shouldNotifyOs(entry, { activeTabId: "tab-B", enabled: true, windowFocused: true })).toBe(true);
  });
  it("stays silent only when the capturing tab is active AND the app window is focused", () => {
    expect(shouldNotifyOs(entry, { activeTabId: "tab-A", enabled: true, windowFocused: true })).toBe(false);
  });
  it("fires when the capturing tab is active but the app window is not focused", () => {
    ...
    expect(shouldNotifyOs(entry, { activeTabId: "tab-A", enabled: true, windowFocused: false })).toBe(true);
  });
  it("stays silent when notifications are disabled", () => {
    expect(shouldNotifyOs(entry, { activeTabId: "tab-B", enabled: false, windowFocused: false })).toBe(false);
  });
});
