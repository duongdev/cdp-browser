...
export interface KeyLike {
  ...
  altKey: boolean
  shiftKey: boolean
  /** Physical-key code (e.g. "BracketLeft"); layout-independent, present on real KeyboardEvents. */
  code?: string
...
export function routeKey(e: KeyLike, ctx: ChatContext, pendingG: boolean): KeyIntent | null {
  ...
  // ⌘⇧[ / ⌘⇧] — previous/next conversation (the macOS prev/next-tab idiom). Shift is required:
  // bare ⌘[/⌘] is history back/forward in browsers and the chat shell. Matched on `code` because
  // Shift turns the character into {/} on most layouts; `key` is the test-only fallback.
  if ((e.metaKey || e.ctrlKey) && !e.altKey && e.shiftKey) {
    if (e.code === "BracketLeft" || e.key === "[" || e.key === "{") return { type: "conv-prev" }
    if (e.code === "BracketRight" || e.key === "]" || e.key === "}") return { type: "conv-next" }
  }
