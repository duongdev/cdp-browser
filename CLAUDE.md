...
- **Renderer** (`src/`): React + Tailwind + shadcn/ui app. Layered on three domain modules in `src/lib/` — Remote Page (single live connection, event demux), Tabs (stable ordering), and Viewport Transform (letterbox math). See `docs/adr/0001-single-remote-page.md` for the single-session constraint.
...
```bash
npm test          # Vitest unit tests (36 tests across src/lib/)
