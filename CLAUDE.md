...
├── notifications-sidechain.js  # Notification Side-Channel state machine + store (createNotificationCenter, DI); used by main.js + server.mjs
├── remote-page-connector.js    # Remote Page connect choreography (createRemotePageConnector, DI); used by server.mjs (main.js adoption tracked)
├── theme-emulation.js   # Pure theme-sync logic (emulatedMediaParams); CommonJS, tested by theme-emulation.test.ts
├── cdp-endpoints.js     # Pure /json URL builders; consumed by main.js + server.mjs
├── settings-store.js    # Pure settings/pins/ui-state store; consumed by main.js + server.mjs
