...
│   ├── notifications-sidechain.js # Notification Side-Channel state machine (createNotificationCenter, DI); Slack-only cred extraction (xoxc+d-cookie) via onCreds dep + listCreds/markCredsStale/setSelfUserId accessors (t069)
...
│   ├── slack-sweep.js             # Pure sweep reducer: planFetches/reduceMessages/applyReadUpdates/isMention/tsCmp; counts+history→entries keyed slack:{team}:{channel}:{ts}, parity+mute+exclude+watermark (t068, ADR-0011)
│   ├── slack-creds.js             # Pure cred helpers: parseLocalConfig (xoxc tokens)/pickDCookie/markFresh/markStale/redact; effectful extraction lives in notifications-sidechain.js (t069, ADR-0011)
│   ├── slack-workspaces.js        # Pure workspace registry + parked-tab planner: upsertWorkspace/liveTeamIds/planParkedTabs; keeper recreates closed Slack tabs (t070, ADR-0011). No creds on disk — re-extracted from any live tab
│   ├── slack-sweep-runner.js      # Effectful sweep orchestrator (createSlackSweeper, DI): per-workspace clientCounts→plan→history→reduce→ingest+read-sync; first-sweep seeding (no cold-start spam); sole authoritative Slack writer (t071). users.counts fallback for team_is_restricted Grid children — seed-to-now + unread-set read-sync (t075). ADR-0011
...
│   ├── notification-health.js     # Pure capture-health aggregator: buildHealth (per-workspace healthy/degraded/unsupported) + shouldAlert (one-time degrade gate); /api/notifications/health (t074, ADR-0011)
