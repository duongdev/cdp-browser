...
└── src/
    ...
    │   ├── cdp-web-transport.ts # Web build: installs window.cdp over POST+SSE+WS, no-op window.local, webCaps (no Electron)
    ...
    │   ├── transport-selector.ts # Web build: pure mode-selection state machine (Auto/WS/Stream/Batch; retry/downgrade/re-probe). See ADR-0007
    │   ├── perf-mark.ts       # Debug-only perf accumulator (tagged stage timings, ?perf=1); remove with [DEBUG-perf] markers once bottleneck is identified
    │   └── utils.ts           # cn() utility
