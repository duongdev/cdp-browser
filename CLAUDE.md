...
- **Unread badges by group**: Sidebar unread counts are computed by `aggregateUnread` (`src/lib/unread-aggregator.ts`) and keyed by `groupKey` (from the notification entry) falling back to URL origin. Every tab/pin of the same app shares one count whether or not it captured the notification, and a dormant pin still badges by its saved URL's origin.
...
│   ├── notifications.js           # Pure notification logic (dedup, cap, OS-toast gating)
